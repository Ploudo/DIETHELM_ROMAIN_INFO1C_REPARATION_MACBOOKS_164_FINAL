...
##### Cette démo. CRUD (Create Read Update Delete) presque complète sur les 3 tables de la base film : Soit "t_film"; "t_ecran" et la table intermédiaire "t_genre_film"
...
A) Modifier MES requêtes AFFICHER (READ) du CRUD(Create Read Update Delete) sur la table "t_genre"
  ...
  * Modifier les requêtes "SELECT" sur la table "t_genres" par des "SELECT" sur une de vos "t_????"
  * Modifier dans le répertoire "TEMPLATES" la page html consacrée à l'affichage pour ma "t_genre", il faut ouvrir "templates/genres/genres_afficher.html"
...
D) Recommencer les étapes A) à C) pour l'action "CREATE" pour la table "t_ecran"

E) Recommencer les étapes A) à C) pour l'action "UPDATE" pour la table "t_ecran"

F) Recommencer les étapes A) à C) pour l'action "DELETE" pour la table "t_ecran"
